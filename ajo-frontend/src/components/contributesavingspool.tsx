...
import { ContributeSavingsPoolProps } from "@/constants";
import { useState, useEffect } from "react";
import { useWriteContract, useReadContract } from "wagmi";
import { savingsPoolAbi, erc20Abi } from "@/abi";
import TransactionStatus from "./transactionstatus";
import { convertAmountToDecimals } from "@/util";

export default function ContributeSavingsPoolPopUp({
  onClose,
  poolAddress,
  tokenAddress,
  contributionAmount,
}: ContributeSavingsPoolProps) {
  ...
  return (
    <div className="min-h-screen w-full fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 overflow-hidden p-0 m-0">
      <div className="bg-white p-6 rounded-md shadow-lg relative w-80 sm:w-96">
        <button
        ...
        {!isPending && !isSuccess && !isError && (
          <div>
            <h2 className="text-lg font-bold mb-6 text-center">Join Pool</h2>
            <form className="flex flex-col w-full">
              <div className="w-full space-y-4">
                <div>
                  <label className="flex items-center text-black text-left mb-1">
                    Contribution Amount
                    <span className="ml-1 relative group">
                      <span className="flex items-center justify-center w-4 h-4 rounded-full border border-black text-black text-xs font-bold cursor-help">
                        ?
                      </span>
                      <span
              ...
              <button
                type="submit"
                className={`bg-blue-50 text-black px-4 py-2 rounded mt-6 w-full transition-colors ${
